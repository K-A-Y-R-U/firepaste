...
@section('title', __('Mis Canjes de Códigos'))
        ...
        <div class="col-12">
            <div class="dashboard-welcome">
                <div class="d-flex align-items-center justify-content-between flex-wrap gap-3">
                    <div>
                        <h4 class="welcome-title mb-1"><i class="bi bi-clock-history me-2" style="color:#667eea;"></i>{{ __('Mis Canjes de Códigos') }}</h4>
                        <p class="welcome-sub mb-0">{{ __('Historial de códigos de regalo canjeados') }}</p>
                    </div>
                    <div class="d-flex gap-2">
                        <a href="{{ route('gift-codes.redeem') }}" class="btn-action">
                            <i class="bi bi-gift me-1"></i> {{ __('Canjear Código') }}
                        </a>
                ...
                <div class="d-flex align-items-center justify-content-between flex-wrap gap-3">
                    ...
                    <div class="d-flex align-items-center gap-4 flex-wrap">
                        <div class="text-center">
                            <div class="vip-stat-label">{{ __('Estado VIP') }}</div>
                            @if($vipStatus['is_active'])
                                <span class="vip-stat-value active">✨ {{ __('Activo') }} ({{ $vipStatus['days_remaining'] }} {{ __('días') }})</span>
                            @else
                                <span class="vip-stat-value inactive">{{ __('Inactivo') }}</span>
                            @endif
                        </div>
                        @if($vipStatus['is_active'])
                            <div class="text-center">
                                <div class="vip-stat-label">{{ __('Expira el') }}</div>
                ...
                <div class="dashboard-card redemption-card">
                    <div class="redemption-header">
                        <div class="d-flex align-items-center gap-3 flex-wrap">
                            ...
                            <span class="days-badge"><i class="bi bi-star-fill me-1"></i>{{ $redemption->giftCode->vip_days }} {{ __('días VIP') }}</span>
                            <span class="redeem-date"><i class="bi bi-calendar me-1"></i>{{ __('Canjeado el') }} {{ $redemption->redeemed_at->format('d/m/Y H:i') }}</span>
                        </div>
                        @if($redemption->isVipActive())
                            <span class="status-badge active"><i class="bi bi-check-circle-fill me-1"></i>{{ __('Activo') }}</span>
                        @else
                            <span class="status-badge expired"><i class="bi bi-x-circle-fill me-1"></i>{{ __('Expirado') }}</span>
                        @endif
                    </div>

                    <div class="redemption-details">
                        <div class="detail-item">
                            <span class="detail-label">{{ __('VIP inició') }}</span>
                            <span class="detail-value">{{ $redemption->vip_starts_at->format('d/m/Y H:i') }}</span>
                        </div>
                        <div class="detail-item">
                            <span class="detail-label">{{ __('VIP termina') }}</span>
                            <span class="detail-value">{{ $redemption->vip_ends_at->format('d/m/Y H:i') }}</span>
                        </div>
                        <div class="detail-item">
                            <span class="detail-label">{{ __('Días restantes') }}</span>
                            <span class="detail-value {{ $redemption->isVipActive() ? 'text-success' : 'text-muted' }}">{{ $remainingDays }} {{ __('días') }}</span>
                        </div>
                    </div>
                    ...
                    @if($redemption->isVipActive())
                        <div class="progress-wrap">
                            <div class="progress-label">
                                <span>{{ __('Progreso') }}: {{ $usedDays }}/{{ $totalDays }} {{ __('días utilizados') }}</span>
        ...
        <div class="row">
            <div class="col-12">
                <div class="dashboard-card empty-state">
                    ...
                    <h5 class="empty-title">{{ __('No has canjeado códigos') }}</h5>
                    <p class="empty-sub">{{ __('Cuando canjees tu primer código de regalo, aparecerá aquí.') }}</p>
                    <a href="{{ route('gift-codes.redeem') }}" class="redeem-btn">
                        <i class="bi bi-gift me-2"></i>{{ __('Canjear mi primer código') }}
                    </a>
                </div>
            </div>
        </div>
